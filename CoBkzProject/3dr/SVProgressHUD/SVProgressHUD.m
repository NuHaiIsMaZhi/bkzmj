...
+ (void)showSuccessWithStatus:(NSString *)string {
    [self sharedView];
    [self showImage:[UIImage imageNamed:@"SucessImage"] status:string];
}

+ (void)showErrorWithStatus:(NSString *)string {
    [self sharedView];
    [self showImage:[UIImage imageNamed:@"ErrorImage"] status:string];
}

+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[SVProgressHUD sharedView] displayDurationForString:string];
    [[self sharedView] showImage:image status:string duration:displayInterval*3];
}
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration
{
    ...
    self.stringLabel.textColor = [UIColor blackColor];
    ...
    _hudView.frame = CGRectMake((self.frame.size.width-350/2)/2,(self.frame.size.height-390/2)/2, 350/2, 390/2);
    _imageView.frame = CGRectMake((_hudView.frame.size.width - _imageView.frame.size.width)/2, 50, _imageView.frame.size.width, _imageView.frame.size.height);
    
    _stringLabel.frame = CGRectMake((_hudView.frame.size.width-_stringLabel.frame.size.width)/2, _imageView.frame.origin.y+_imageView.frame.size.height+30, _stringLabel.frame.size.width, _stringLabel.frame.size.height);
}
...
- (UIImageView *)imageView {
    if (_imageView == nil)
    {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 222/2, 148/2)];
        _imageView.layer.transform = CATransform3DMakeScale(0.8, 0.8, 1);
    }
